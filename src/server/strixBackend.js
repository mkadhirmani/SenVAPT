...
export async function isSafeWebhookUrl(urlStr) {
  ...
  try {
    ...
    const hostname = parsed.hostname.toLowerCase();
    // Allow trusted Sennovate corporate gateways (e.g. n8n on OpenShift / Sennovate internal network)
    if (isTrustedSennovateCorporateHost(hostname)) {
      return true;
    }
    if (!(/^(\d{1,3}\.){3}\d{1,3}$/.test(hostname) || hostname.includes(':'))) {
...
/**
 * Strip all ANSI terminal color and escape sequences from logs
 */
...
export function parseLocalStrixFolder(folderPath) {
  ...
  // Check strix log / scan.log
  if (!actualTargetUrl && (raw.strix_log || raw.log_tail)) {
    const logStr = stripAnsi(raw.strix_log || raw.log_tail);
    const logM = logStr.match(/(?:Target|Target\s+Domain)\s+([a-zA-Z0-9\.\-]+)/i) ||
                 logStr.match(/strix\s+-t\s+["']?([^"'\s]+)["']?/i) ||
                 logStr.match(/target:\s*["']?([^"'\s\n]+)["']?/i);
    if (logM && logM[1] && !logM[1].includes('target.com')) {
      const cleanTarget = logM[1].trim();
      actualTargetUrl = cleanTarget.startsWith('http') ? cleanTarget : `https://${cleanTarget}`;
    }
  }
 ...
 * reads scan.log for completed Strix output paths, and navigates
...
export function resolveStrixOutputFolderFromExtract(extractDir, minStartTimeMs = 0, targetDomain = '', previousRunId = null) {
      ...
      for (const item of items) {
        ...
        try {
          ...
          if (st.isDirectory()) {
            ...
            mainScanLogs.push({ path: full, mtime: st.mtimeMs || 0 });
          } else if (item === 'strix.log' || item.endsWith('.log')) {
            otherLogs.push({ path: full, mtime: st.mtimeMs || 0 });
          }
        } catch (_) {}
      }
    } catch (_) {}
  ...
  // Sort logs by newest mtime first
  mainScanLogs.sort((a, b) => b.mtime - a.mtime);
  ...
  // 3. Analyze log files to check whether the scan is completed and locate its output folder & telemetry
  ...
  for (const logPath of logFiles) {
    try {
      ...
      let lastCompLineIdx = -1;
      let lastInitLineIdx = -1;

      for (let i = 0; i < filteredLines.length; i++) {
        const line = filteredLines[i];
        if (compRegex.test(line)) lastCompLineIdx = i;
      ...
      if (lastInitLineIdx !== -1) {
        const initBoxLines = filteredLines.slice(lastInitLineIdx, Math.min(filteredLines.length, lastInitLineIdx + 40));
        const initText = initBoxLines.join('\n');
        ...
        if (!initiatedRunOutputFolder) {
          const directMatch = initText.match(/(?:Output|run_dir)\s*[:|=]?\s*['"]?(?:strix_runs\/|\/?root\/[^\/]+\/strix_runs\/|\/?[^\s\r\n│\t,)"']+\/strix_runs\/)?([a-zA-Z0-9_\-\.]+)/i);
          if (directMatch && directMatch[1] && directMatch[1].toLowerCase() !== 'tokens') {
            initiatedRunOutputFolder = directMatch[1].trim();
            initiatedRunFullPath = buildServerScanOutputPath(detectedTargetDomain || targetDomain, initiatedRunOutputFolder);
          }
        }
      }

      // Parse output folder from completed section ONLY if a fresh completion occurred AFTER initiation
      let completedRunOutputFolder = null;
      let completedRunFullPath = null;
      const isFreshCompletion = lastCompLineIdx !== -1 && (lastInitLineIdx === -1 ? minStartTimeMs === 0 : lastCompLineIdx > lastInitLineIdx);
      ...
      // Slicing live log lines and parsing telemetry:
      // If an initiation was found, the current scan ONLY consists of lines starting at lastInitLineIdx!
      // This guarantees that past scan completions and old findings do not pollute the current scan.
      const currentScanLines = lastInitLineIdx !== -1 ? filteredLines.slice(lastInitLineIdx) : filteredLines;
      liveLogLines = currentScanLines.slice(-1000);
      liveLogTail = liveLogLines.join('\n');

      const currentScanText = currentScanLines.join('\n');
      const inTokenMatch = currentScanText.match(/Input Tokens\s*([\d\.]+\s*[kKMGT]?)/i);
      const cachedTokenMatch = currentScanText.match(/Cached Tokens\s*([\d\.]+\s*[kKMGT]?)/i);
      const outTokenMatch = currentScanText.match(/Output Tokens\s*([\d\.]+\s*[kKMGT]?)/i);
      const costMatch = currentScanText.match(/Cost\s*\$?([\d\.]+)/i);

      if (inTokenMatch) parsedTokens += parseMultiplier(inTokenMatch[1]);
      if (cachedTokenMatch) parsedTokens += parseMultiplier(cachedTokenMatch[1]);
      ...
      const targetMatch = currentScanText.match(/(?:Target|Target\s+Domain)\s+(?:(?:\d+)\s+targets\s*)?([a-zA-Z0-9\.\-]+)/i);
      if (targetMatch && targetMatch[1] && targetMatch[1].includes('.') && !targetMatch[1].startsWith('.')) {
        detectedTargetDomain = targetMatch[1].trim();
      } else {
        const targetIdx = currentScanLines.findIndex(l => /Target\s+/i.test(l));
        if (targetIdx !== -1) {
          for (let j = targetIdx; j < Math.min(currentScanLines.length, targetIdx + 8); j++) {
            const domM = currentScanLines[j].match(/([a-zA-Z0-9][a-zA-Z0-9\-]*\.[a-zA-Z]{2,}(?:\.[a-zA-Z]{2,})?)/);
            if (domM) {
      ...
      // Determine completion status from scan.log:
      // If the completion banner appears after initiation (or at the end of the log), the scan is complete.
      const hasCompletedBanner = lastCompLineIdx !== -1 && (lastInitLineIdx === -1 || lastCompLineIdx > lastInitLineIdx);

      if (hasCompletedBanner) {
        isLogCompleted = true;
        isLogActive = false;
        ...
        // Initiation exists, but no completion banner yet: the scan is ACTIVELY RUNNING!
  ...
  const findDirectoryByName = (dir, targetName, depth = 0) => {
    if (depth > 8 || !targetName) return null;
    try {
      const items = fs.readdirSync(dir);
      for (const item of items) {
        const sub = path.join(dir, item);
        try {
          ...
          if (st.isDirectory()) {
            if (item.toLowerCase() === targetName.toLowerCase()) return sub;
            const res = findDirectoryByName(sub, targetName, depth + 1);
            if (res) return res;
          }
        } catch (_) {}
      }
  ...
  // 4. SCAN RESOLUTION
  if (minStartTimeMs > 0 || isLogCompleted || isLogActive) {
    const targetRunName = latestRunOutputFolder || initiatedRunOutputFolder;

    // If the log confirms completion of the scan:
    if (isLogCompleted) {
      ...
      if (!resolvedDir && targetRunName) {
        ...
        if (matchCand) resolvedDir = matchCand.dir;
      }

      // If not resolved by exact folder name, match candidateDirs for finalized folder
      if (!resolvedDir && candidateDirs.length > 0) {
        const matchCand = candidateDirs.find(c => c.targetMatch && c.isFinalized) ||
                          candidateDirs.find(c => c.isFinalized) ||
                          candidateDirs[0];
        if (matchCand) resolvedDir = matchCand.dir;
      }

      const hasArtifacts = resolvedDir && (
        fs.existsSync(path.join(resolvedDir, 'run.json')) ||
        fs.existsSync(path.join(resolvedDir, 'penetration_test_report.md')) ||
        fs.existsSync(path.join(resolvedDir, 'findings.sarif')) ||
        fs.existsSync(path.join(resolvedDir, 'vulnerabilities.json')) ||
        fs.existsSync(path.join(resolvedDir, 'vulnerabilities'))
      );

      if (resolvedDir && hasArtifacts) {
        const effectiveName = targetRunName || path.basename(resolvedDir);
        return {
          bestDir: resolvedDir,
          folderName: effectiveName,
          outputFullPath: latestRunFullPath || buildServerScanOutputPath(effectiveDomain, effectiveName),
          isScanning: false,
          inProgress: false,
          scanFinished: true,
          freshFound: true,
      ...
      // The log indicates completion, but the target run folder has not yet synced into the archive.
      return {
        bestDir: null,
        folderName: targetRunName || targetDomain,
        outputFullPath: latestRunFullPath || buildServerScanOutputPath(effectiveDomain, targetRunName || targetDomain),
        isScanning: true,
        inProgress: true,
        isStalled: false,
        ...
        scanFinished: false,
        freshFound: false,
        activeRunId: targetRunName,
        liveLogLines,
        strixLog: liveLogTail,
        tokens: parsedTokens,
        cost: parsedCost,
        message: `Strix audit completed on server (${targetRunName}); finalizing report artifacts...`
      };
    }
    ...
    return {
      ...
      liveLogLines: (activeRunId || liveLogLines.length > 0) ? liveLogLines : [],
      strixLog: (activeRunId || liveLogTail.length > 0) ? liveLogTail : '',
  ...
  if (latestRunOutputFolder) {
    ...
    const foundDirect = findDirectoryByName(extractDir, latestRunOutputFolder);
    if (foundDirect) {
      return {
        bestDir: foundDirect,
...
export async function fetchN8nScanResultsProxy(payload) {
  ...
  try {
    ...
    if (resolvedResult.tokens > 0 && (!parsed.tokens || parsed.tokens === 0)) {
      parsed.tokens = resolvedResult.tokens;
    }
    if (resolvedResult.cost > 0 && (!parsed.cost || parsed.cost === 0)) {
      parsed.cost = resolvedResult.cost;
    }
...
export function getScanSession(scanId) {
  ...
  if (session.status === 'running' && session.logs && session.logs.length > 0) {
    ...
    for (const line of recentLogs) {
      if (completionRegex.test(stripAnsi(line))) {
